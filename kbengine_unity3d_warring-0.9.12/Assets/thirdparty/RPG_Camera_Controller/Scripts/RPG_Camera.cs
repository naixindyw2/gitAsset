...
public class RPG_Camera : MonoBehaviour {
    ...
    public static void CameraSetup() {
        GameObject cameraUsed = null;
        GameObject cameraPivotObj = null;
        RPG_Camera cameraScript = null;
        ...
        if (Camera.main != null)
        {
			...
			/*
        	cameraUsed = GameObject.Find("Main Camera") as GameObject;
        	
        	if(cameraUsed == null)
				cameraUsed = new GameObject("Main Camera");
			
            if (!cameraUsed.GetComponent("Camera"))
            {
        ...
        cameraPivotObj = GameObject.Find("cameraPivot") as GameObject;

        if(cameraPivotObj != null)
      	  cameraScript.cameraPivot = cameraPivotObj.transform;
    }
    
    
    void LateUpdate() {
        if (cameraPivot == null) {
            Debug.Log("Error: No cameraPivot found! Please read the manual for further instructions. id=" + name);
            CameraSetup();
            return;
        }
    ...
    void GetInput() {
        ...
        if (Input.GetMouseButton(0) || Input.GetMouseButton(1)) {
            // CustomCursor.showCursor = false; // if you want the cursor behavior of the version 1.0, change this line to "Screen.lockCursor = true;"
        ...
        {
            // CustomCursor.showCursor = true; // if you want the cursor behavior of the version 1.0, change this line to "Screen.lockCursor = false;"
    	}
        ...
        if (Input.GetMouseButton(1) && RPG_Controller.enabled)
    ...
    void CharacterFade() {
        if (RPG_Animation.instance == null || RPG_Animation.instance.GetComponent<Renderer>() == null)
            return;
    ...
    float CheckCameraClipPlane(Vector3 from, Vector3 to) {
        ...
        if (Physics.Linecast(from, to, out hitInfo) && hitInfo.collider.tag != "Player" && hitInfo.collider.tag != "kbentity")
            closestDistance = hitInfo.distance - Camera.main.nearClipPlane;
        
        if (Physics.Linecast(from - transform.right * halfPlaneWidth + transform.up * halfPlaneHeight, clipPlane.UpperLeft, out hitInfo) && hitInfo.collider.tag != "Player" && hitInfo.collider.tag != "kbentity")
        ...
        if (Physics.Linecast(from + transform.right * halfPlaneWidth + transform.up * halfPlaneHeight, clipPlane.UpperRight, out hitInfo) && hitInfo.collider.tag != "Player" && hitInfo.collider.tag != "kbentity")
            if (hitInfo.distance < closestDistance || closestDistance == -1)
                closestDistance = Vector3.Distance(hitInfo.point - transform.right * halfPlaneWidth - transform.up * halfPlaneHeight, from);
        
        if (Physics.Linecast(from - transform.right * halfPlaneWidth - transform.up * halfPlaneHeight, clipPlane.LowerLeft, out hitInfo) && hitInfo.collider.tag != "Player" && hitInfo.collider.tag != "kbentity")
            if (hitInfo.distance < closestDistance || closestDistance == -1)
                closestDistance = Vector3.Distance(hitInfo.point + transform.right * halfPlaneWidth + transform.up * halfPlaneHeight, from);
        
        if (Physics.Linecast(from + transform.right * halfPlaneWidth - transform.up * halfPlaneHeight, clipPlane.LowerRight, out hitInfo) && hitInfo.collider.tag != "Player" && hitInfo.collider.tag != "kbentity")
            if (hitInfo.distance < closestDistance || closestDistance == -1)
                closestDistance = Vector3.Distance(hitInfo.point - transform.right * halfPlaneWidth + transform.up * halfPlaneHeight, from);
